...
func TestSumShaError(t *testing.T) {
	data := []byte("Hoge")
	_, err := shasum(&data, "512256256")
	if err == nil {
		t.Error("not error is error")
	}

	errstr := fmt.Sprintf("%s", err)
	if errstr != "unsupported algorithm" {
		t.Error("undefined error", errstr)
	}
}
